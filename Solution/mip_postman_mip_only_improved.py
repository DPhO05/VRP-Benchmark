...
try:
    ...
    extra = os.path.join(os.getcwd(), "benchmark", ".deps")
...
def solve(n, k, dist, limit_sec):
    ...
    for r in range(k):
        ...
        solver.Add(sum(y[i, r] - sum(x[i, j, r] for j in range(1, n + 1) if i != j) for i in range(1, n + 1)) == depot_out)
...
def main():
    ...
    limit = float(os.environ.get("VRP_TIME_LIMIT_SEC", "4.0"))
    if "--time-limit" in sys.argv:
        limit = float(sys.argv[sys.argv.index("--time-limit") + 1])
    start = time.time()
